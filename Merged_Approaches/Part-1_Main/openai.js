 ...
 */
async function generateAndSaveOpenAIImage(
  prompt,
  creativeId,
  ...
  try {
    console.log(
      `💬 [OpenAI] Starting image generation for creative ${creativeId} with prompt: "${prompt}"`
    );
    ...
    try {
      ...
      console.error(
        `❌ [OpenAI] Error from OpenAI API during generation for creative ${creativeId}:`,
        apiError.message
      );
      if (apiError.response && apiError.response.status) {
        console.error(
          `   Status: ${apiError.response.status}, Data:`,
    ...
    if (!imageUrl) {
      console.error(
        `❌ [OpenAI] No image URL found in OpenAI response for creative ${creativeId}. Response data:`,
        response.data
      );
    ...
    try {
      ...
      if (!imageResponse.ok) {
        console.error(
          `❌ [OpenAI] Failed to fetch generated OpenAI image for creative ${creativeId}: HTTP Status ${imageResponse.status}, Text: ${imageResponse.statusText}`
        );
        throw new Error(
          `Failed to fetch generated OpenAI image: ${imageResponse.statusText}`
        );
      }
      console.log(`✅ [OpenAI] Image fetched successfully from URL.`);
    } catch (fetchError) {
      console.error(
        `❌ [OpenAI] Network error during image fetch for creative ${creativeId}:`,
        fetchError.message
      );
    ...
    console.log(
      `✅ [OpenAI] Image buffer converted. Size: ${imageUint8Array.length} bytes.`
    );
    ...
    console.log(
      `📤 [OpenAI] Attempting upload to Supabase Storage bucket "${bucketName}" at "${filePath}"`
    );
    ...
    if (uploadError) {
      console.error(
        `❌ [OpenAI] Supabase upload failed for creative ${creativeId}:`,
        uploadError.message
      );
    ...
    console.log(
      `✅ [OpenAI] Image uploaded successfully to Supabase. Public URL: ${publicUrl}`
    );

    // --- STEP 6: Update creative record with image URL ---
    console.log(
      `💬 [OpenAI] Attempting to update Supabase table 'creatives_duplicate' for creative ${creativeId}`
    );
    const { error: updateError } = await supabase
      .from("creatives_duplicate")
      .update({
        chat_imagery: {
          // Update a separate 'chat_imagery' column for chat-specific images
          url: publicUrl,
          alt_text: prompt,
    ...
    if (updateError) {
      console.error(
        `❌ [OpenAI] Failed to update creative record for OpenAI image: ${updateError.message}`
      );
      console.error(`   Supabase update error details:`, updateError); // Log full error object
    } else {
      console.log(
        `✅ [OpenAI] Creative record updated successfully for creative ${creativeId}.`
      );
    }
    ...
    console.error(
      `❌ [OpenAI] Overall Image generation failed for creative ${creativeId}:`,
      error.message
    );
    return {
      success: false,
