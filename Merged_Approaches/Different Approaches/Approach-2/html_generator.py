...
import cv2
import easyocr
import numpy as np # Import numpy here as it's used directly in image decoding
...
# REPLICATE_MODEL constant is removed as Replicate generation is no longer done here.
    ...
    mapped_data = {
        ...
        "Canvas": {
            ...
            "Imagery": {
                ...
                "full_poster_image_url": None # NEW: To store the full AI-generated image URL
            },
            "Text_Blocks": [],
            "cta_buttons": [],
            "brand_logo": {},
...
# ------------------------------------------------------
# Phase 1: Generate Full Creative Image using Replicate (REMOVED)
# ------------------------------------------------------
# The generate_full_creative function is no longer needed here,
# as the full creative image URL will be fetched from Supabase.
...
def extract_text_positions(image_bytes_io):
    """
    Extracs text and their bounding box positions from an image provided as bytes (in-memory)
    using EasyOCR.
    """
...
def main():
    try:
        # Replicate client initialization and token validation removed, as it's no longer used for generation here.
        ...
        # Extract the full creative image URL (type: "poster") for OCR
        print("Extracting full_creative_url (type: 'poster') for OCR.", file=sys.stderr)
        ...
        if not full_creative_url:
            raise ValueError("Error: Full creative image URL (type: 'poster') not found in Supabase data. Cannot perform OCR.")


        # Phase 1: (PREVIOUSLY GENERATE) Now: Retrieve and Download the full creative image (for OCR)
